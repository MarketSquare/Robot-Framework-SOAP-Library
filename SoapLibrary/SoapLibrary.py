...
class SoapLibrary:
    ...
    def create_soap_client(self, url):
        ...
        logger.info('Connected to: ' + self.client.wsdl.location)
        info = self.client.service.__dict__
        operations = info["_operations"]
        logger.info('Available operations: ' + str(operations.keys()))

    @keyword("Call SOAP Method With XML")
    def call_soap_method_xml(self, xml):
        ...
        if status_code != 200:
            logger.debug('URL: ' + str(response.url))
            logger.debug(etree.tostring(etree_response, pretty_print=True, encoding='unicode'))
            raise AssertionError('Request Error! Status Code: ' + str(status_code) + '! Reason: ' + str(response.reason))
        logger.info('<font size="3"><b>Response from webservice:</b></font> ', html=True)
    ...
    def get_data_from_xml_tag(self, xml, tag, index=1):
        ...
        if len(data_list) == 0:
            logger.warn('The search "' + xpath + '" did not return any result! Please confirm the tag!')
        if len(data_list) > 1:
            logger.warn('The tag you entered found ' + str(len(data_list)) + ' items, returning the text in the index '
                        'number ' + str(new_index) + ', if you want a different index inform the argument index=N')
        return data_list[new_index].text

    @keyword("Edit XML Request")
    def edit_xml(self, xml_filepath, new_values_dict, request_name):
        ...
        """
        string_xml = self._convert_xml_to_raw_text(xml_filepath)
        xml = self._convert_string_to_xml(string_xml)
        if not isinstance(new_values_dict, dict):
            raise Exception("new_values_dict must be a dictionary")
        for key, value in new_values_dict.iteritems():
            if len(xml.xpath("//*[name()='%s']" % key)) == 0:
                logger.warn('Tag ' + key + ' not found')
                continue
            xml.xpath("//*[name()='%s']" % key)[0].text = value
        # Create new file with replaced request
        new_file_name = str(request_name) + ".xml"
        new_file_path = os.path.join(os.path.dirname(xml_filepath), new_file_name)
    ...
    def save_xml_response_file(self, response, response_name):
